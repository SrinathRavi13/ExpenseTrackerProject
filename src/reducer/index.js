...
const expenseReducer = (state = intialState, action) => {
    switch (action.type) {
        ...
        case actionTypes.EDIT_INDEX:
            return action.id
        case actionTypes.EXPENSE_ITEM: 
            return state.expenseList.filter(e => e.id !== action.id);
        case actionTypes.UPDATE_EXPENSE:
            let expense = action.expense;
            let expenseList = state.expenseList.map((item) => {
                return (item.id === expense.id) ? Object.assign({}, item, expense) : item;
            })
            return {
                ...state,
        ...
        case actionTypes.DELETE_EXPENSE:
            let filteredObj = state.expenseList.filter(e => e.id !== action.id);
            return {
                ...state,
